...
Appendix C is certified by a frozen transcript (`transcript.jsonl`) and a machine-readable run summary (`summary.json`)
that records the exact parameters and the transcript SHA256.
...
## 4) “Appendix-by-appendix…” — insérer une nouvelle section Appendix C

### Où
Dans `## Appendix-by-appendix: ...`, **insère avant** `# Appendix D — Case B finite sanity scan ...` (donc après la fin de A.4).
...
## 5) “About `lehmer_bnb.py` …” — supprimer l’avertissement obsolète

### Où
Dans la section `## About lehmer_bnb.py (finite verification infrastructure)`, **supprime** le bloc :

> **Important repository note.**  
> The essential artefacts currently included…  
> If your paper’s Appendix C claims…  
> …Appendix C would not be independently verifiable…

Et remplace-le par :

```md
**Appendix C linkage.**  
The repository ships the Appendix C certificate artefacts (`certificates/appendixC/summary.json` and
`certificates/appendixC/transcript.jsonl`). The checker mode of `lehmer_bnb.py` replays the transcript deterministically
and returns `PASS` (exit code 0) when the certificate verifies.
